...
#Generate all products in particular given slug and return no of active slug
def get_Active_Product(category_slug):
    res=requests(url=velocityUrl+"get_total_active_product_category_wise/"+category_slug,headers=headers)
    return len(res.data)
...
def create_Report_and_send_By_Email(categorylist):
     ...
     if os.path.exists("Report_Feedback"):
        ...
        masterDataFrame.to_csv("Active_Report_In_Category/Active_Product_In_Category.csv",index=False,header=True, encoding='utf-8')

        if os.path.exists("Active_Report_In_Category/Active_Product_In_Category.csv"):
            print("Report created succesfully....")
            msg = MIMEMultipart()
            msg['From'] = "proscons12345@gmail.com"
            msg['To'] = "4b158cdb.shorthillstech.com@in.teams.ms"
            msg['Subject'] = "Auto Generated Less than 5 Live Product in From all category List"
            part = MIMEBase('application', "octet-stream")
            part.set_payload(open("Active_Report_In_Category/Active_Product_In_Category.csv", "rb").read())
            ...
            return "Active_Report_In_Category/Active_Product_In_Category.csv"
        else:
            return "Report not generated"             
           
def analyseActiveorLiveProduct():
    listOfLessThan5=[]
    category_list=get_All_category()
    for category in category_list:
        noOfActive=get_Active_Product(category['category_slug'])
        if noOfActive>0 and noOfActive<5:
            temp={
                'id':category['id'],
                'name':category['name'],
            ...
            listOfLessThan5.append(temp)
    
    if listOfLessThan5==[]:
        print("No any category whose number of live product is less than 5")
    else:
        create_Report_and_send_By_Email(listOfLessThan5) 

schedule.every(10).hours.do(analyseActiveorLiveProduct)
while True:
